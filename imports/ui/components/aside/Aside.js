...
class NavigationItem extends Component {
  ...
  renderTopLevelItem = item => {
    ...
    const routesList = item.subItems.map(item =>
      typeof item.route === "string" ? item.route : item.route.name
    );
    const curRouteName = FlowRouter.getRouteName();
    return (
      <li
        className={classNames("top-nav-item", {
          active: curRouteName === item.route
        })}
      >
        <div
          onClick={() => this.toggleTopMenu()}
          className={classNames("nav-item-label", {
            active:
              routesList.indexOf(curRouteName) > -1 ||
              curRouteName === item.route
          })}
        >
          {item.label}
        </div>
        <ul className="sub-nav-items" ref={node => (this.subMenu = node)}>
          {subItemList}
        </ul>
      </li>
    );
  ...
  renderLowLevelItem = (item, index) => {
    ...
    const isActive =
      typeof item.route === "string"
        ? curRouteName === item.route
        : curRouteName === item.route.name &&
          FlowRouter.current().params.id === item.route.params.id;
    return (
      <li
    ...
    return (
      <li
        ...
        className={classNames("nav-item", {
          active: FlowRouter.getRouteName() === route
        })}
      >
        {label}
      </li>
...
class Aside extends React.Component {
  constructor(props) {
    ...
    if (settingsItemIndex) {
      ...
      if (
        Roles.userIsInRole(props.currentUser._id, [ROLES.ADMIN, ROLES.SALES])
      ) {
