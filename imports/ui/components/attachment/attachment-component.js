...
class AttachmentComponent extends Component {
  ...
  _onDragStart = event => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      const DownloadURL = `${file.contentType}:${path.basename(
        filePath
      )}:file://${filePath}`;
  ...
  _onClickSaveToDrive = event => {
    if (
      !this.picker ||
      this.state.isSavingFileToDrive ||
      this.props.file.isBackedUp
    )
      return;
    this.picker.pick(({ docs }) => {
      ...
      Meteor.call(
        "Nylas.saveFileToGoogle",
        { fileId: this.props.file.id, folderId },
        () => {
  ...
  _renderRemoveIcon() {
    return <img src="/icons/attachments/remove-attachment.png" />;
  }

  _renderDownloadButton() {
    return <img src="/icons/attachments/icon-attachment-download.png" />;
  }

  _renderGoogleDriveButton() {
    return <img src="/icons/attachments/drive.png" width="50%" />;
  }

  _renderLoadingIcon() {
    return <i className="fa fa-circle-o-notch fa-spin fa-fw" />;
  }
  ...
  _renderSaveToGoogleDrive(className) {
    ...
    return (
      <div
        className={`file-action-icon ${className}`}
        onClick={this._onClickSaveToDrive}
      >
        {renderIcon()}
      </div>
    );
        ...
        >
          <span className={`progress-bar-wrap state-${downloadState}`}>
            ...
            <span
              ...
              style={this._downloadProgressStyle()}
            />
          </span>
          ...
          >
            <div className="file-info-wrap">
              ...
              <span className="file-name">
                {NylasUtils.displayNameForFile(file)}
              </span>
              <span className="file-size">
                {NylasUtils.displayFileSize(file)}
              </span>
            </div>
