...
class ImageAttachmentComponent extends AttachmentComponent {
  ...
  _renderImage() {
    ...
    if (file && file.blob) {
      ...
      return (
        <img
          id={`image-${download.fileId}`}
          ref="previewImageAttachment"
          src={download.blob}
        />
      );
    } else {
  ...
  _renderRemoveIcon() {
    return <img src="/icons/attachments/image-cancel-button.png" />;
  }

  _renderDownloadButton() {
    return <img src="/icons/attachments/image-download-button.png" />;
  }

  _onLoadImage = e => {
    if (this.refs.previewImageAttachment)
      this.refs.previewImageAttachment.src = e.target.result;
  };
