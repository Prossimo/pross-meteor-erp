...
export default class PeopleList extends TrackerReact(React.Component) {
    ...
    getSortedData() {
        ...
        if (asc) {
            return sortBy(people, by)
        } else {
            return sortBy(people, by).reverse()
        }
    }
        ...
        return (
            <Grid fluid>
                <Row>
                    <Col md={4}>
                        <Card>
                            <CardHeader>Social Info</CardHeader>
                            <CardBody>
    ...
    onToggleRow = (person) => {
        ...
        const index = findIndex(expanded, {_id:person._id})
        if(index > -1) expanded.splice(index, 1)
        else expanded.push(person)

        this.setState({expanded})
    }
    isExpanded = (person) => findIndex(this.state.expanded, {_id:person._id}) > -1
