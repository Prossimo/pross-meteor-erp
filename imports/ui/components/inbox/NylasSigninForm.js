...
export default class NylasSigninForm extends React.Component {
    ...
    return (
      <div style={{ maxWidth: 500, minWidth: 400 }}>
        <h3 style={{ textAlign: "center" }}>
          Add {this.props.isAddingTeamInbox ? "a Team" : "an Individual"} Inbox
        </h3>
        <br />
        <Form horizontal onSubmit={this.onSubmitSignIn}>
          <FormGroup controlId="formHorizontalName">
            ...
            <span style={validationStyle}>{validation.name}</span>
          </FormGroup>
          <FormGroup controlId="formHorizontalEmail">
            ...
            <span style={validationStyle}>{validation.email}</span>
          </FormGroup>
          ...
          <FormGroup controlId="formHorizontalPassword">
            ...
            <span style={validationStyle}>{validation.password}</span>
          </FormGroup>
          ...
          <FormGroup>
            ...
            <Col sm={9}>
              <FormControl
                ...
                value={provider}
                onChange={evt => this.setState({ provider: evt.target.value })}
                disabled={isProcessing}
              >
                <option value="">-----</option>
            ...
            <span style={validationStyle}>{validation.provider}</span>
          </FormGroup>
          ...
          <FormGroup>
            <Col smOffset={3} sm={10}>
              ...
              &nbsp;
              <Button type="submit" bsStyle="primary" disabled={isProcessing}>
                {isProcessing && <i className="fa fa-refresh fa-spin" />}
                &nbsp;Sign in
              </Button>
            </Col>
          </FormGroup>
  ...
  onSubmitSignIn = evt => {
    ...
    if (provider == "gmail") {
      //console.log('gmail inbox redirectUri', config.google.redirectUri)
      ...
      window.addEventListener(
        "message",
        this.receiveMessageFromGoolgeAuthWindow,
        false
      );
    } else {
      this.signin();
  ...
  signin = () => {
    ...
    Meteor.call("addNylasAccount", signinData, (err, res) => {
      console.log("Signin to Inbox", err);
      if (err) {
        console.log(err);
                ...
                (error, response, body) => {
                  ...
                  if (!error && body) {
                    ...
                    if (googleEmail != this.signinData.email) {
                      return warning(
                        "Registraion email is different from Google authentication email!"
                      );
                    } else {
