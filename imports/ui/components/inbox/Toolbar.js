...
export default class Toolbar extends TrackerReact(React.Component) {
    ...
    return (
      <div className="toolbar-panel">
        ...
        <div style={{ flex: 1 }}>
          ...
          <Selector
            ...
            triggerEl={
              <Button disabled={!thread}>
                {assignee ? (
                  <div>
                    <span style={{ fontSize: 11 }}>Assigned to </span>
          ...
          />
          &nbsp;&nbsp;&nbsp;
          <Button disabled={!thread} onClick={this.onToggleFollow}>
            {thread &&
            thread.followers &&
            thread.followers.indexOf(Meteor.userId()) > -1
              ? "Unfollow"
              : "Follow"}
          </Button>
        </div>
        <div>
          {(threadTotalCount > 0 || draftTotalCount > 0) &&
            this.renderPagingButtons()}
        </div>
        <div>{this.renderExtraMenu()}</div>
      </div>
    );
  }
    ...
    return (
      <div style={{ marginLeft: "auto", marginRight: 10 }}>
        <span>
          ...
          {isDrafts
            ? draftTotalCount
            : _.min([threadStartIndex + PAGESIZE, threadTotalCount])}{" "}
          of {isDrafts ? draftTotalCount : threadTotalCount}
        </span>
        <Button
          disabled={threadStartIndex - PAGESIZE < 0 || isDrafts}
          onClick={this.props.onPrevPage}
        >
          <i className="fa fa-chevron-left" />
        </Button>
        <Button
          disabled={threadStartIndex + PAGESIZE >= threadTotalCount || isDrafts}
      ...
      return (
        <div>
          <DropdownButton
            bsStyle="default"
            bsSize="small"
            title={salesRecord.name}
            pullRight
            id="dropdown-deal"
          >
            <MenuItem
              onSelect={() =>
                this.props.onSelectExtraMenu("goto", {
                  type: "deal",
                  _id: salesRecord._id
                })
              }
            ...
            <MenuItem divider />
            <MenuItem onSelect={() => this.props.onSelectExtraMenu("unbind")}>
              Unbind from this deal
            </MenuItem>
          </DropdownButton>
        </div>
      );
    ...
    return (
      <div style={{ float: "right" }}>
        ...
        >
          <MenuItem
            onSelect={() =>
              this.props.onSelectExtraMenu("create", { type: "deal" })
            }
          >
            Create new deal from this thread
          </MenuItem>
          <MenuItem divider />
          ...
          <MenuItem header>
            <InputGroup>
              ...
              <FormControl
                type="text"
                placeholder="Search..."
                onChange={this.onChangeSearchSalesRecord}
              />
            </InputGroup>
          </MenuItem>
          {salesRecords.map(sr => (
            <MenuItem
              key={sr._id}
              onSelect={() =>
                this.props.onSelectExtraMenu("bind", { type: "deal", doc: sr })
              }
            >
              {sr.name}
            </MenuItem>
        ...
        >
          <MenuItem
            onSelect={() =>
              this.props.onSelectExtraMenu("create", { type: "project" })
            }
          >
            Create new project from this thread
          </MenuItem>
          <MenuItem divider />
          ...
          {projects.map(pr => (
            <MenuItem
              key={pr._id}
              onSelect={() =>
                this.props.onSelectExtraMenu("bind", {
                  type: "project",
                  doc: pr
                })
              }
  ...
  onChangeSearchSalesRecord = evt => {
    ...
    this.searchTimeout = setTimeout(() => {
      if (keyword.length) {
        this.setState({
          salesRecords: SalesRecords.find(
  ...
  onSelectAssignee = assignee => {
    const { thread } = this.props;
    if (assignee && thread.assignee && assignee.value === thread.assignee)
      return;
