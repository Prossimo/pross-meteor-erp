...
import FileUploadStore from '/imports/api/nylas/file-upload-store'
...
export default class ComposeView extends React.Component {
   ...
   constructor(props) {
      ...
      this.state = {
         ...
         downloads: FileDownloadStore.downloadDataForFiles(map(draft.downloads, 'id')),
   ...
   componentDidMount() {console.log('ComposeView componentDidMount')
      ...
      // For saving drafts every 1 minute
      this.saveDraftInterval = setInterval(() => {
         this._onSaveDraft()
      }, 1 * 1000 * 60)   // every 1 minute
   ...
   componentWillUnmount() {console.log('ComposeView componentWillUnmount')
      ...
      if (this.saveDraftInterval) clearInterval(this.saveDraftInterval)
   }
   ...
   _renderQuotedBody() {
      ...
      return (
         <div>
            ...
            {!hideQuotedBody &&
            <div className="signature"><EmailFrame showQuotedText={true} content={quotedBody}/></div>}
         </div>
      )
   }
   ...
   _onSendDraft = () => {
      if (!this._isValidDraft()) return

      Actions.sendDraft(this.props.clientId)
   }
