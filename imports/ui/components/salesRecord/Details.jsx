...
import {
  removeClientStatus,
  removeSupplierStatus
} from "/imports/api/models/salesRecords/verified-methods";
...
class Details extends TrackerReact(React.Component) {
        ...
        return (
          <Select
            ...
            onChange={
              onChange
                ? onChange
                : ({ value }) => this.changeState(field, value, true)
            }
            onBlur={this.saveSalesRecord}
      ...
      case "currency":
        return (
          <input
            ...
            style={{ width: "100%" }}
            onChange={event =>
              this.changeState(field, parseFloat(event.target.value))
            }
            onBlur={this.saveSalesRecord}
        ...
        return (
          <div>
            <DatePicker
              ...
              onChange={date =>
                this.changeStateElem(field, date ? date.toDate() : null, 0)
              }
              onBlur={this.saveSalesRecord}
            />
            &nbsp; to &nbsp;
            <DatePicker
              ...
              endDate={value && value.length > 0 && moment(value[1])}
              onChange={date =>
                this.changeStateElem(field, date ? date.toDate() : null, 1)
              }
              onBlur={this.saveSalesRecord}
            />
          </div>
    ...
    return _.map(rows, ({ type, field, label, readonly }) => {
      ...
      if (type === "select") {
        if (field === "stage") {
          ...
          selectOptions = SHIPPING_MODE_LIST.map(value => ({
            label: value,
            value
          }));
          ...
          selectOptions = Object.values(DEAL_PRIORITY).map(value => ({
            label: value,
            value
          }));
        } else if (field === "probability") {
          selectOptions = Object.values(DEAL_PROBABILITY).map(value => ({
            label: value,
            value
          }));
        } else if (field === "clientStatus") {
          const statuses = ClientStatus.find().fetch();
          selectOptions = statuses.map(s => ({
            label: s.name,
            value: s._id,
            editable: s.editable
          }));
          ...
          selectOptions = Object.values(STATES).map(state => ({
          ...
          const companyType = CompanyTypes.findOne({
          ...
          const stakeholderDesignation = PeopleDesignations.findOne({
            name: "Stakeholder"
          });
          const stakeholders = People.find({
            designation_id: stakeholderDesignation._id
          }).fetch();
          selectOptions = stakeholders.map(p => ({
            value: p._id,
            label: p.name,
            email: p.defaultEmail(),
            phone: p.defaultPhoneNumber()
          }));
              ...
              <div>
                {field === "clientStatus" &&
                  Roles.userIsInRole(Meteor.userId(), [ROLES.ADMIN]) && (
                    <button
                      ...
                      onClick={this.handleAddClientStatus}
                    >
                      +
                    </button>
                  )}
                {field === "supplierStatus" &&
                  Roles.userIsInRole(Meteor.userId(), [ROLES.ADMIN]) && (
                    <button
                      className="btn btn-default"
                      onClick={this.handleAddSupplierStatus}
                    >
                      +
                    </button>
                  )}
      ...
      if (type === "daterange")
        displayValue = `from ${moment(_.first(value)).format(
          "MM/DD/YYYY"
        )} to ${moment(_.last(value)).format("MM/DD/YYYY")}`;
      if (type === "currency")
  ...
  render() {
    ...
    const attrRows = [
      ...
      {
        label: "Act. Production Start Date",
      ...
      { label: "Forwarder", field: "shipper", type: "select" },
      {
        ...
        field: "estProductionTime",
        type: "number"
      },
      { label: "Est. Lead time", field: "estLeadTime", type: "number" },
      {
        label: "Est. Production completion",
        field: "estProductionCompletion",
        type: "date"
      },
      {
        label: "Est. Delivery Window",
        field: "estDeliveryRange",
        type: "daterange"
      },
      {
        label: "Act. Client Payment Received",
        field: "actClientPaymentReceived",
        type: "date"
      },
      {
        label: "Act. Production Completion",
        field: "actProductionCompletion",
        type: "date"
      },
    ...
    const shippingRows = [
      {
        label: "Contact name",
        field: "shippingContactPersonId",
        type: "select"
      },
      {
        label: "Contact email",
        field: "shippingContactEmail",
        type: "display",
        readonly: true
      },
      {
    ...
    return (
      <div className="details-inbox-tab">
        ...
        <div className="row">
          <div className="col-md-6">
            <div className="panel panel-default">
              <div className="panel-heading">Deal Status</div>
          ...
          <div className="col-md-6">
            <div className="panel panel-default">
              <div className="panel-heading">Production/Delivery Info</div>
              <div className="panel-body">
                <table className="table table-condensed">
                  <tbody>
        ...
        <div className="row">
          ...
          <div className="col-md-6">
            <div className="panel panel-default">
              <div className="panel-heading">Billing</div>
              <div className="panel-body">
                <table className="table table-condensed">
                  <tbody>
        ...
        <div className="panel panel-default">
          <div className="panel-heading">Others</div>
          <div className="panel-body">
            <table className="table table-condensed">
              <tbody>
