...
class AllSalesRecords extends React.Component {
    ...
    this.state = {
      ...
      possibleColumns: [
        ...
        },
        {
          ...
          selected: false,
          options: record =>
            record.getMembers().map(m => ({ value: m._id, label: m.name() })),
          type: "select",
          editable: true,
          renderer: record => {
        ...
        },
        {
          ...
          label: "Priority",
          selected: false,
          options: Object.values(DEAL_PRIORITY).map(v => ({
            value: v,
            label: v
          })),
          type: "select",
        ...
        },
        {
          ...
          renderer: record => {
            ...
            return `$ ${parseFloat(record.expectedRevenue).toLocaleString(
              "en-US",
              {
        ...
        },
        {
          ...
          label: "Probability",
          selected: false,
          options: Object.values(DEAL_PROBABILITY).map(v => ({
            value: v,
            label: v
          })),
          type: "select",
        ...
        },
        {
          ...
          editable: true,
          renderer: record => {
            const status = _.findWhere(clientStatuses, {
              _id: record.clientStatus
            });
        ...
        },
        {
          ...
          editable: true,
          renderer: record => {
            const status = _.findWhere(supplierStatuses, {
              _id: record.supplierStatus
            });
            return status ? status.name : null;
          }
        },
  ...
  renderSaveButton = key => {
    return (
      <button
        ...
        onClick={
          key === "stage" ? () => this.updateStage(key) : this.updateProject
        }
      >
  ...
  getSortedData = () => {
    ...
    if (!this.state.showArchivedDeals)
      salesRecords = salesRecords.filter(s => !s.archived);
    if (keyword && keyword.length > 0) {
      salesRecords = salesRecords.filter(
        s => s.name.toLowerCase().indexOf(keyword.toLowerCase()) > -1
      );
    }
      ...
      <tr key={project._id}>
        ...
        <td>
          <div className="btn-group">
            <Button onClick={() => this.goToProject(project)} bsSize="small">
            ...
            {Roles.userIsInRole(Meteor.userId(), ROLES.ADMIN) &&
              !project.archived && (
                <Button
                  onClick={() => this.archiveSalesRecord(project._id)}
                  bsStyle="warning"
                  bsSize="small"
                >
            ...
            {Roles.userIsInRole(Meteor.userId(), ROLES.ADMIN) &&
              project.archived && (
                <Button
                  onClick={() => this.activeSalesRecord(project._id)}
                  bsStyle="success"
                  bsSize="small"
                >
                  <i className="fa fa-archive" />
      ...
      <div className="flex toolbar-wrapper">
        <div className="margin-auto">
          <input
            ...
            onChange={e =>
              this.setState({ showArchivedDeals: e.target.checked })
            }
          />
          &nbsp;Show Archived Deals&nbsp;&nbsp;
        </div>
  ...
  render() {
    ...
    return (
      <Panel collapsed={collapsed}>
        <Panel.HeadingNavbar>{this.renderNavbar()}</Panel.HeadingNavbar>
