...
class AddQuoteForm extends React.Component {
  ...
  constructor(props) {
    ...
    const defaultRevisionNumber = _.isArray(props.quote.revisions)
      ? props.quote.revisions.length
      : 0;
  ...
  formSubmit(event) {
    ...
    const needUpdate = quote.revisions.some(
    ...
    const slackMsgParans = {
      ...
      attachments: [
        {
          color: "#36a64f",
          text: `<${FlowRouter.url(FlowRouter.current().path)}|Go to project ${
            salesRecord.name
          }>`
        }
      ]
    };
    const slackText = `I just ${
      needUpdate ? "updated" : "added"
      ...
      <div className="add-quote-form">
        <form className="default-form" onSubmit={this.formSubmit.bind(this)}>
          ...
          <div className="field-wrap">
            ...
            <label htmlFor="form-revision-quote-file" className="file-label" />
            <input
              type="file"
          ...
          <div className="field-wrap">
            ...
            <textarea onChange={this.noteChange} value={note} rows={4} />
          </div>
          ...
          {draftClientId && (
            <div>
              ...
              />
              <label htmlFor="alert-checkbox" className="check-label">
                Alert stakeholders
              </label>
              {alertsActive &&
                (NylasUtils.hasNylasAccounts() ? (
                  <TemplateSelect
                    onChange={this.onSelectMailTemplate}
                    selectedTemplate={selectedMailTemplate}
                  />
                ) : (
                  <Alert bsStyle="warning">
                    You need to set inbox to email!
                  </Alert>
                ))}
          ...
          <div className="submit-wrap">
            {this.state.isUploading ? (
              <button className="btnn primary-btn">
