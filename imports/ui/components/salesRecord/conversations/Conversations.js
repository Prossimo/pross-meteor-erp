...
export default class Conversations extends TrackerReact(React.Component) {
    ...
    render() {
        ...
        const tabTitleNode = (c) => {
            if(c.name !== 'Main' && (Roles.userIsInRole(Meteor.userId(), ROLES.ADMIN) || c.owner === Meteor.userId())) {
                return <div className="delete-conversation-wrapper">{c.name}<i className="fa fa-close" onClick={() => this.deleteConversation(c)}/></div>
            } else {
                return c.name
            }
        }
