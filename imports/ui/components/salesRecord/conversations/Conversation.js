...
export default class Conversation extends TrackerReact(React.Component) {
  ...
  renderComposeModal() {
    ...
    if (
      ...
      draft.conversationId != this.props.conversationId
    )
      return "";

    return (
      <ComposeModal
        isOpen={composeState.show}
  ...
  renderParticipantsSelectModal() {
    const participants = this.props.onlyStakeholders
      ? this.target()
          .people()
          .filter(
            p => p.designation() && p.designation().name === "Stakeholder"
          )
      : this.target().people();

    const selections = this.conversation().participants.filter(
      p => typeof p === "object"
    );
    ...
    return (
      <ParticipantsSelectModal
        show={showParticipantsSelectModal}
  ...
  render() {
    ...
    const participants = Conversations.findOne(
      conversationId
    ).getParticipants();
        ...
        <div style={{ display: "flex" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <ParticipantList
              participants={participants}
              onAddParticipant={() =>
                this.setState({ showParticipantsSelectModal: true })
              }
