...
export default class ParticipantsSelector extends React.Component {
    ...
    selectParticipant = (p, selected) => {
        ...
        const index = findIndex(selections, {peopleId:p._id})
        if(selected && index == -1) {
        ...
        return (
            <Table bordered condensed hover>
                ...
                <tbody>
                {
                    participants.map(p => (
                        <tr key={p._id}>
                            <td><Checkbox onChange={(e) => {this.selectParticipant(p, e.target.checked)}} checked={findIndex(selections,{peopleId:p._id}) > -1}/></td>
                            <td>{p.name}</td>
