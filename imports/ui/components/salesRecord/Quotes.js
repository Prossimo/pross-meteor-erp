...
class QuoteItem extends React.Component {
  ...
  renderQuoteName(quote) {
    ...
    if (
      !Roles.userIsInRole(Meteor.userId(), [ROLES.ADMIN, ROLES.SALES]) &&
    ...
    return (
      <p className="title">
        <span
          className={classNames("quote-name", { hide: editQuoteNameMode })}
        ...
        <input
          ...
          className={classNames("quote-name-input", {
            active: editQuoteNameMode
          })}
        />
      </p>
    );
  ...
  render() {
    const { quote } = this.props;
    const lastRevisionNumber = Math.max(
      ...quote.revisions.map(item => item.revisionNumber)
    );
    const latest = _.findWhere(quote.revisions, {
    ...
    const revisions = quote.revisions
      ...
      .map(revision => (
        <li className="revision-item" key={revision.fileId}>
          <p>
            Revision
          ...
          <p>
            Total price{" "}
            {currencyFormatter.format(revision.totalPrice, {
              code: "USD",
              locale: "en-US",
              decimalDigits: 1
            })}
        ...
        <div className="flex-container">
          <div className="desc-part">
            ...
            <p className="quote-info">
              ...
              Revision{" "}
              <span className="revision-label"> # {latest.revisionNumber}</span>
            </p>
            <p className="quote-info">
              <i className="fa fa-calendar-o" />{" "}
              {moment(latest.createAt).format("MMMM Do YYYY")}
            </p>
            <p className="quote-info">
              Total price:{" "}
              {currencyFormatter.format(latest.totalPrice, {
                code: "USD",
                locale: "en-US",
                decimalDigits: 0
              })}
          ...
          <div className="control-part">
            <button
              onClick={this.addRevision.bind(this, quote)}
              className="btnn primary-btn"
            >
...
class Quotes extends React.Component {
    ...
    this.slideToggle = function(event) {
      ...
      if ($(quote).hasClass("up")) {
        ...
        $(quote)
          ...
          .slideDown();
        $(quote)
          .parents(".single-quota")
          .siblings(".single-quota")
          .find(".revision-list")
          .slideUp();
        $(quote)
          .parents(".single-quota")
  ...
  showAddQuoteForm() {
    ...
    const to = salesRecord
      .contactsForMainParticipants()
      .map(c => ({ name: c.name, email: c.email }));
    if (to && to.length) {
      DraftStore.createDraftForQuoteEmail({
        to,
        conversationId:
          salesRecord.conversationIds && salesRecord.conversationIds[0]
      }).then(draft => {
        this.setState({
          showPopup: true,
  ...
  renderPopup() {
    ...
    return (
      ...
      >
        ...
        <Modal.Body>{popupData}</Modal.Body>
      </Modal>
    );
  }
  ...
  renderAddQuotes() {
    ...
    return (
      <div className="add-quotes">
        <button
          onClick={this.showAddQuoteForm.bind(this)}
          className="btnn primary-btn"
        >
          Add quote
        </button>
  ...
  getGoogleDriveFileList() {
    ...
    return (
      <div className="add-quotes">
        <button
          onClick={this.getDriveFileList.bind(this)}
          className="btnn primary-btn"
        >
          Get google drive list of files
        </button>
      </div>
    );
  }
