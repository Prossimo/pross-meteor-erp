    ...
    <li key={index}>
      <InputGroup>
        <FormControl
          ...
          {...{ "data-value": value }}
          defaultValue={label}
          onKeyPress={this.updateStatus}
          onChange={this.setEditState}
        />
        <InputGroup.Addon onClick={() => this.delStatus(value)}>
    ...
    return (
      <div>
        <Dropdown id={`${key}-${record._id}`}>
          <Dropdown.Toggle>{current.label || "Select status"}</Dropdown.Toggle>
