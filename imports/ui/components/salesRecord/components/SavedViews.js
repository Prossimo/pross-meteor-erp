...
class SavedViews extends Component {
  ...
  selectState = event => {
    const state = _.find(this.state.states, {
      id: event.currentTarget.dataset.stateid
    });
    if (state) {
  ...
  removeState = id => {
    ...
    Meteor.call("users.dealsState.remove", id, (err, res) => {
      ...
      this.setState({
        states: [].concat(states.slice(0, dropId), states.slice(1 + dropId))
      });
    });
  };
          ...
          <Modal.Body>
                ...
                .map((state, index) => (
                  <li key={index}>
                    ...
                    <span
                      ...
                      onClick={event => {
                        event.preventDefault();
                        return this.removeState(state.id);
                      }}
                    >
            ...
            <InputGroup>
              <FormControl
                ...
                onChange={this.newStateName}
                ref={node => (this.input = node)}
                value={newStateName}
              />
              <InputGroup.Addon onClick={this.saveNew}>
