...
export default class SelectMembers extends Component {
  ...
  constructor(props) {
    ...
    const selectedMembers = props.members
      ? Users.find({ _id: { $in: props.members } }).map(u => ({
          label: u.name(),
          value: u._id,
          username: u.username,
          roles: u.roles.join(", ")
  ...
  render() {
    const members = Users.find()
      ...
      .filter(
        ({ _id, status, slack }) =>
          Roles.userIsInRole(_id, [ROLES.ADMIN, ROLES.SALES, ROLES.MANAGER]) &&
    ...
    return (
      <div className="panel panel-default">
        <div className="panel-heading">Add Members</div>
