...
class CreateSalesRecord extends TrackerReact(React.Component) {
  ...
  constructor(props) {
    super(props);
    this.shippingMode = SHIPPING_MODE_LIST.map(item => ({
      label: item,
      value: item
    }));
    this.stages = STAGES.map(item => ({
    ...
    this.state = {
      ...
      subStage: salesRecord
        ? salesRecord.subStage
        : this.getFirstSubStage(STAGES[0]),
      actualDeliveryDate: salesRecord
        ? moment(salesRecord.actualDeliveryDate)
        : null /*moment()*/,
      productionStartDate: salesRecord
        ? moment(salesRecord.productionStartDate)
        : null /*moment()*/,
      startDate:
        salesRecord &&
        salesRecord.estDeliveryRange &&
        salesRecord.estDeliveryRange.length
          ? moment(salesRecord.estDeliveryRange[0])
          : null /*moment().subtract(29, 'days')*/,
      endDate:
        salesRecord &&
        salesRecord.estDeliveryRange &&
        salesRecord.estDeliveryRange.length
          ? moment(salesRecord.estDeliveryRange[1])
          : null /*moment()*/,
      ...
      selectedShippingMode: salesRecord
        ? _.find(this.shippingMode, { value: salesRecord.shippingMode })
        : this.shippingMode[0],
      selectedStage: salesRecord
        ? _.find(this.stages, { value: salesRecord.stage })
        : this.stages[0],
      ...
      selectedConversation:
        salesRecord &&
        salesRecord.conversationIds &&
        salesRecord.conversationIds.length > 0
          ? salesRecord.conversationIds[0]
          : null,
    ...
    if (props.thread) {
      ...
      const people = salesRecord
        ? People.find({
            _id: { $in: _.pluck(salesRecord.stakeholders, "peopleId") }
          })
            .fetch()
            .map(p => {
              const stakeholder = salesRecord.stakeholders.find(
                s => s.peopleId === p._id
              );
              return _.extend(p, {
        ...
        : [];
      const threadPeople = People.find({
        "emails.email": {
          $in: _.pluck(participants, "email").filter(
            email => !NylasUtils.isOwner(account_id, email)
          )
        }
      }).fetch();
  ...
  submitForm(event) {
    ...
    const data = {
      ...
      actualDeliveryDate: actualDeliveryDate
        ? actualDeliveryDate.toDate()
        : undefined,
      productionStartDate: productionStartDate
        ? productionStartDate.toDate()
        : undefined,
      estDeliveryRange:
        startDate && endDate
    ...
    if (salesRecord) {
      Meteor.call(
        ...
        (err, res) => {
          ...
          if (err)
            return warning(
              `Problems with updating new SalesRecord. ${err.error}`
            );
      ...
      Meteor.call(
        "insertSalesRecord",
        { data, thread, isPrivateSlackChannel },
        (err, res) => {
          this.props.toggleLoader(false);
          if (err)
            return warning(
              `Problems with creating new SalesRecord. ${err.error}`
            );
  ...
  render() {
    const { projectName, subStage, selectedStage, members } = this.state;
    const { stages } = this;
    ...
    switch (this.props.stage) {
      case "lead":
        ...
        break;
      case "opportunity":
        submitBtnName = this.props.salesRecord
          ? "Save Opportunity"
          : "Add Opportunity";
      ...
      <div className="create-project">
        <form onSubmit={this.submitForm.bind(this)} className="">
            ...
            <div className="col-md-4">
              <SelectSubStage
                ...
                onSelectSubStage={subStage =>
                  this.setState({ subStage: subStage.value })
                }
              />
            </div>
          </div>
          ...
          {!this.props.salesRecord && (
            <div className="form-group">
              <Checkbox
                checked={this.state.isPrivateSlackChannel}
                onChange={e =>
                  this.setState({ isPrivateSlackChannel: e.target.checked })
                }
              >
                Is Private Slack Channel
              </Checkbox>
            </div>
          )}
  ...
  renderConversationSelector() {
    ...
    const conversations = Conversations.find({
      _id: { $in: salesRecord.conversationIds }
    }).fetch();
      ...
      <div className="panel panel-default">
        <div className="panel-heading">
          <div className="panel-header">
            ...
            <div className="action" onClick={this.onClickAddConversation}>
              +
            </div>
          </div>
        </div>
        <div className="panel-body" style={{ display: "flex" }}>
          <FormGroup>
            {conversations.map(c => (
              <Radio
                key={`conversation-radio-${c._id}`}
                value={c._id}
                checked={selectedConversation == c._id}
                onChange={this.selectConversation}
                inline
              >
                {" "}
                {c.name}
