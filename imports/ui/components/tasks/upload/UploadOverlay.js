  ...
  componentDidMount() {
    ...
    Meteor.call("drive.getAccessToken", {}, (error, token) => {
      if (error) {
        return swal(
          "Google Drive",
          "could not connect to google drive to attach files to current task",
          "error"
        );
      } else {
        ...
        taskElem.ondrop = event => {
          ...
          const willUploadFiles = _.toArray(files).filter(
            f => !(!f.type && f.size % 4096 == 0)
          );
            ...
            const uploader = new MediaUploader({
              ...
              onProgress: ({ loaded, total }) => {
                ...
                const totalPercentage =
                  percentages.reduce((sum, next) => sum + next, 0) /
                  willUploadFiles.length;
                ...
                if (completedSize === willUploadFiles.length) {
                  ...
                  Meteor.call(
                    "task.attachFiles",
                    {
                      ...
                      attachments: completedFiles.map(
                        ({ id, name, mimeType }) => ({
                          _id: id,
                          name,
                          mimeType
