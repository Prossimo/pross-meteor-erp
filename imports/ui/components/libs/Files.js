...
function getParentFolder(fileId) {
  let i = 0,
    j;
  for (; i < folder_ct; i++) {
    if (fileview_data[i].data.id == fileId) return fileview_data[i].data;
    for (
      j = 0;
      fileview_data[i].data.children &&
      j < fileview_data[i].data.children.length;
      j++
    ) {
...
function merge_fileview_data() {
  ...
  for (; i > 0; i--) {
    ...
    for (j = 0; j < fileview_data[parent_idx].data.children.length; j++) {
      if (
        fileview_data[parent_idx].data.children[j].id ==
        fileview_data[i].data.id
      )
        fileview_data[parent_idx].data.children[j].children =
          fileview_data[i].data.children;
    }
  }
    ...
    <div style={style.base}>
      ...
      {!props.node.preventRemove ? (
        <a
          ...
          style={{ marginLeft: 20 }}
          onClick={event =>
            props.removeFile(props.node.id, props.node.name, event)
          }
        >
...
class Files extends Component {
  ...
  componentDidMount() {
    ...
    Meteor.call(
      "drive.listFiles",
      { query: `'${this.state.folderId}' in parents and trashed = false` },
      (error, result) => {
  ...
  updateSelectedFolder() {
    ...
    Meteor.call(
      "drive.listFiles",
      { query: `'${curnode.id}' in parents and trashed = false` },
      (error, result) => {
        if (error || !result || !result.files) {
          return warning("could not list files from google drive");
        }
  ...
  addGooglefiles(event) {
    ...
    if (
      !this.state.cursor ||
      this.state.cursor.mimeType != "application/vnd.google-apps.folder"
    ) {
    ...
    Meteor.call("drive.createFile", file, (error, result) => {
      ...
      switch (filetype) {
        case "application/vnd.google-apps.document":
          file_url = `https://www.googleapis.com/drive/v3/files/${
            this.state.cursor.id
          }/export?mimeType=application/vnd.openxmlformats-officedocument.wordprocessingml.document&alt=media&access_token=${
            this.token
          }`;
          break;
        case "application/vnd.google-apps.spreadsheet":
          file_url = `https://www.googleapis.com/drive/v3/files/${
            this.state.cursor.id
          }/export?mimeType=application/vnd.openxmlformats-officedocument.spreadsheetml.sheet&alt=media&access_token=${
            this.token
          }`;
          break;
        case "application/vnd.google-apps.presentation":
          file_url = `https://www.googleapis.com/drive/v3/files/${
            this.state.cursor.id
          }/export?mimeType=application/vnd.openxmlformats-officedocument.presentationml.presentation&alt=media&access_token=${
            this.token
          }`;
          break;
        case "application/vnd.google-apps.drawing":
          file_url = `https://www.googleapis.com/drive/v3/files/${
            this.state.cursor.id
          }/export?mimeType=application/pdf&alt=media&access_token=${
            this.token
          }`;
          break;
      }
        ...
        <div className="row">
          ...
          <div className="col-md-4">
            <div className="text-center">
              ...
              <label htmlFor="quote-file" className="file-label" />
              <input
                type="file"
                ref="file"
                id="quote-file"
            ...
            <div className="text-center">
              ...
              <input
                ...
                onClick={this.addGooglefiles}
                data-val="Docs"
                value="Add Google Docs file"
              />
            </div>
            <div className="text-center">
              ...
              <input
                ...
                className="hand-cursor"
                onClick={this.addGooglefiles}
                data-val="Spreadsheet"
                value="Add Google SpreadSheet file"
              />
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <input
                ...
                onClick={this.addGooglefiles}
                data-val="Drawing"
                value="Add Google Drawing file"
              />
            </div>
            <div>{this.renderAttachFiles()}</div>
          </div>
        </div>
      </div>
    );
