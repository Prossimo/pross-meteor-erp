...
import { getSlackUsername, getAvatarUrl } from "/imports/api/lib/filters";
...
class Files extends Component {
      ...
      Meteor.call("drive.getFiles", { fileId: folderId }, error => {
        if (error) {
          ...
          switch (type) {
            case "project":
              return Meteor.call(
                "drive.createProjectFolder",
                { projectId: _id, newName },
                this.listFiles
              );
            case "salesRecord":
              return Meteor.call(
                "drive.createSalesRecordFolder",
                {
                  salesRecordId: _id,
                  newName
                },
  ...
  listFiles() {
    ...
    Meteor.call("drive.listFiles", { query }, (error, result) => {
  ...
  selectFile(id, name, mimeType, webContentLink, webViewLink) {
    if (this.state.selectedFile.id === id) {
      ...
      this.setState({
        selectedFile: { id, mimeType, webContentLink, webViewLink, name }
      });
    }
  }
      ...
      <ol className="breadcrumb">
        {this.state.viewPath.map(({ folderId, name }) => {
          if (folderId === activeFolderId)
            return (
              <li className="active" key={folderId}>
                {name}
              </li>
            );
          return (
    ...
    return files.map(file => {
      ...
      return (
        <FileRow
          ...
          onClick={() =>
            this.selectFile(id, name, mimeType, webContentLink, webViewLink)
          }
        >
          <td>
      ...
      Meteor.call(
        ...
        error => {
          if (!error) {
            ...
            const slackText = `I just removed the file named as "${
      ...
      <FileManager>
        ...
        <div className="text-center">
          <div className="btn-group">
            <button
              className={`btn btn-default btn-sm fa fa-chevron-left ${
                this.state.viewPath.length <= 1 ? "disabled" : ""
              }`}
              onClick={this.goBack}
            />
            <button
              className="btn btn-default btn-sm fa fa-upload"
              onClick={this.pickFile}
            />
            <button
              className={`btn btn-default btn-sm fa fa-download ${
            ...
            />
            <button
              className={`btn btn-default btn-sm fa fa-trash ${
                !this.state.selectedFile.id ? "disabled" : ""
              }`}
              onClick={this.deleteFile}
            />
            <button
              className="btn btn-default btn-sm fa fa-file"
