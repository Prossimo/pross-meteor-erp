...
export default class Selector extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        if(!isEqual(this.state.value, newProps.value)) {
    ...
    onSelectItem = (item) => {
        ...
        if(this.props.multiple) {
            const index = map(value, 'value').indexOf(item.value)
                ...
                <tbody>
                {
                    options.map((o, i) => (
                        <tr key={i} onClick={() => this.onSelectItem(o)}>
                            <td>{value&&(this.props.multiple ? map(value, 'value').indexOf(o.value)>-1 : value.value == o.value) ? <i className="fa fa-check"/> : <span style={{width:12,height:12}}>&nbsp;</span> }</td>
                            <td>{o.label}</td>
                        </tr>
                    ))
                }
