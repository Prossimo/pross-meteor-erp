...
class EditableUsersTable extends Component {
  ...
  customConfirm = (next, userIds) => {
    if (
      confirm(
        `(It's a custom confirm)Are you sure you want to delete ${userIds}?`
      )
    ) {
  ...
  actionActiveUser = (cell, row) => {
    ...
    return (
      <Button
        onClick={() => this.activeUser(row._id)}
        disabled={!row.slackInvited && !row.slack}
      >
        Active
      </Button>
    );
  ...
  actionSlackInvite = (cell, row) => {
    ...
    return (
      <Button onClick={() => this.inviteUserToSlack(row.email)}>
