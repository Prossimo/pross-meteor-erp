...
import {
  insertCompany,
  updateCompany
} from "/imports/api/models/companies/methods";
...
export default class CompanyForm extends React.Component {
  ...
  render() {
    ...
    const types =
      type_ids && type_ids.length
        ? CompanyTypes.find({ _id: { $in: type_ids } })
            .fetch()
            .map(t => ({ value: t._id, label: t.name }))
        : null;
    return (
      <div>
        <Form style={{ padding: 10 }} horizontal onSubmit={this.onSubmit}>
          ...
          <FormGroup controlId="formHorizontalType">
            ...
            <Col sm={8}>
              <Select
                ...
                value={types}
                onChange={items =>
                  this.setState({ type_ids: _.pluck(items, "value") })
                }
                options={typeOptions}
                clearable={true}
              />
            ...
            <Col sm={1}>
              <Button onClick={this.onClickAddCompanyType}>
          ...
          <AddressInput
            addresses={addresses}
            onChange={data => this.setState({ addresses: data })}
          />
          <PhoneNumberInput
          ...
          <FormGroup>
            <Col sm={12} style={{ textAlign: "right" }}>
              <Button type="submit" bsStyle="primary">
                {this.props.company ? "Update" : "Create"}
              </Button>
            </Col>
          </FormGroup>
        </Form>
  ...
  renderCompanyTypeModal() {
    ...
    const title =
      selectedCompanyType && !creatingType
        ? "Edit Company Type"
        : "Create Company Type";
  ...
  onSubmit = evt => {
    ...
    if (
      website &&
      website.length > 0 &&
      website.indexOf(HTTP_PROTOCOL) == -1 &&
      website.indexOf(HTTPS_PROTOCOL) == -1
    )
    ...
    try {
      ...
      if (onSaved)
        onSaved(Companies.findOne({ _id: companyId }), company != null);
    } catch (e) {
