...
class FixedPopover extends Component {
  ...
  componentDidMount() {
    ...
    findDOMNode(this.refs.popoverContainer).addEventListener(
      "animationend",
      this.onAnimationEnd
    );
  ...
  computeAdjustedOffsetAndDirection = ({
    ...
    const { overflows, overflowValues } = this.computeOverflows({
      currentRect,
      windowDimensions
    });
    const overflowCount = Object.keys(_.pick(overflows, val => val === true))
      .length;
    ...
    if (overflowCount > 0) {
      ...
      const isVerticalDirection = [Directions.Up, Directions.Down].includes(
        direction
      );
      ...
      if (isHorizontalDirection && (overflows.top || overflows.bottom)) {
        const overflowVal = overflows.top
          ? overflowValues.top
          : overflowValues.bottom;
      ...
      if (isVerticalDirection && (overflows.left || overflows.right)) {
        const overflowVal = overflows.left
          ? overflowValues.left
          : overflowValues.right;
      ...
      case Up:
        ...
        popoverStyle = {
          ...
          transform: `translate(${offset.x ||
            0}px) translate(-50%, calc(-100% - 10px))`,
      ...
      case Left:
        ...
        popoverStyle = {
          ...
          transform: `translate(0, ${offset.y ||
            0}px) translate(calc(-100% - 10px), -50%)`,
    ...
    return (
      <div>
        ...
        >
          ...
          <div
            className={`fixed-popover-pointer shadow`}
            style={pointerStyle}
          />
        </div>
      </div>
