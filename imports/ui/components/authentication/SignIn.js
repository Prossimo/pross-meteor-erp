...
class SignIn extends React.Component {
  ...
  submit(e) {
    ...
    Meteor.loginWithPassword({ email }, password, err => {
      if (err) {
        switch (err.error) {
          case "202":
            return this.setState({ authError: "User is in pending status" });
          default:
            return this.setState({ authError: "Invalid username or password" });
        }
      }
  ...
  render() {
    console.log("Prev Path", Session.get("prevPath"));
    ...
    return (
      <div className="sign-in-wrap">
        ...
        <form className="auth-form" onSubmit={this.submit.bind(this)}>
          ...
          <button className="btnn login-btn" type="submit">
            Login
          </button>
          {this.state.authError ? (
            <span className="warn-msg">{this.state.authError}</span>
          ) : (
            ""
          )}
        </form>
        <footer className="auth-footer">
          <button onClick={() => this.toggle("ForgotPassword")}>
            Forgot password
          </button>
          &nbsp;&nbsp;&nbsp;
          <button onClick={() => this.toggle("SignUp")} className="toggle-auth">
            Create new account
          </button>
        </footer>
      </div>
