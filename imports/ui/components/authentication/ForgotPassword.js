...
import { isValidEmail } from "../../../api/lib/validation";
    ...
    return (
      <div className="sign-in-wrap">
        ...
        <form className="auth-form" onSubmit={this.submit}>
          ...
          <button
            className="btnn login-btn"
            type="submit"
            disabled={submitting}
          >
            {submitting && <i className="fa fa-spinner fa-spin fa-fw"> </i>}
            Submit
          </button>
          {this.state.error ? (
            <span className="warn-msg">{this.state.error}</span>
          ) : (
            ""
          )}
        </form>
        <footer className="auth-footer">
          <button onClick={this.toggle.bind(this)} className="toggle-auth">
            I thought password
          </button>
        </footer>
      </div>
