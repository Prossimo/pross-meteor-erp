...
Migrations.add({
  ...
  up() {
    ...
    clientStatuses.forEach(status =>
      ClientStatus.insert({ name: status, editable: false })
    );
    ...
    supplierStatuses.forEach(status =>
      SupplierStatus.insert({ name: status, editable: false })
    );
  },
  down() {}
});
  ...
  up() {
    ...
    salesRecords.forEach(s => {
      ...
      const mainConversationId = Conversations.insert({
        name: "Main",
        participants: (s.stakeholders || [])
          .filter(({ peopleId }) => peopleId != null)
          .map(({ peopleId, isMainStakeholder }) => ({
            peopleId,
            isMain: isMainStakeholder
          }))
      });
      SalesRecords.update(s._id, {
        $set: {
          conversationIds: [mainConversationId].concat(
    ...
    projects.forEach(p => {
      ...
      Projects.update(p._id, {
        $set: { conversationIds: [mainConversationId] }
      });
    });
  },
  down() {
...
Migrations.add({
  ...
  up() {
    const nylasAccounts = _.uniq(
      NylasAccounts.find({ isTeamAccount: true }).fetch(),
      false,
...
Migrations.add({
  version: 10,
  name:
    "Change slack channel field schema on salesrecord and project collections",
  up() {
    const {
      data: { ok, channels }
...
Migrations.add({
  ...
  up() {
    ...
    projects.forEach(project => {
      if (!project.members)
        Projects.update({ _id: project._id }, { $set: { members: [] } });
    });
  },
  down() {
    Projects.remove({ nylasAccountId: { $ne: null } });
...
Migrations.add({
  version: 12,
  name:
    "Remove slackChanel and slackChannelName fields from salesRecord and project",
  up() {
    SalesRecords.find({
      $or: [{ slackChanel: { $ne: null } }, { slackChannelName: { $ne: null } }]
    }).map(({ _id }) => {
      SalesRecords.update(
        { _id },
        { $unset: { slackChanel: 1, slackChannelName: 1 } }
      );
    });
    Projects.find({
      $or: [{ slackChanel: { $ne: null } }, { slackChannelName: { $ne: null } }]
