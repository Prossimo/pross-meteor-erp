...
class Upload {
  ...
  constructor({ clientId, file }) {
    ...
    const token = AccountStore.tokenForAccountId(
      DraftStore.draftForClientId(clientId).account_id
    );
    ...
    const self = this;
    axios
      .post("https://api.nylas.com/files", data, {
        auth: {
          username: token,
          password: ""
        },
...
class FileUploadStoreClass extends Reflux.Store {
  ...
  _onAddAttachment = ({ clientId, file }) => {
    console.log("_onAddAttachment", clientId, file);
    if (!clientId)
      throw new Error(
        "You need to pass the ID of the message (draft) this Action refers to"
      );
  ...
  _verifyFile = file => {
    if (file.size > 25 * 1000000)
      return Promise.reject(
        new Error(
          `${file.name} can not be attached because it is larger than 25MB.`
        )
      );
