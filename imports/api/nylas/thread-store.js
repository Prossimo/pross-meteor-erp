  ...
  onLoadThreads = (category, { page = 1, search } = {}) => {
    ...
    const loadThreads = folder => {
      ...
      return new Promise((resolve, reject) => {
        let path;
        if (this.keyword) {
          path = `/threads/search?${QueryString.stringify(
            Object.assign(query, { q: this.keyword })
          )}`;
        } else {
          path = `/threads?${QueryString.stringify(
            Object.assign(query, { in: folder.id })
          )}`;
    ...
    if (
      ...
      category.id === "not_filed" ||
      category.id === "no_vendors" ||
      category.id === "unreads"
    ) {
      promises = Meteor.user()
        .nylasAccounts()
        .map(account =>
          loadThreads(_.findWhere(account.categories, { name: "inbox" }))
        );
