...
export default class SyncbackDraftFilesTask extends Task {
    ...
    return this.uploadAttachments()
      ...
      .catch(err => {
        ...
        if (
          ...
          !NylasAPI.PermanentErrorCodes.includes(err.statusCode)
        ) {
  ...
  uploadAttachment = upload => {
    console.log("Started uploadAttachment");
  ...
  applyChangesToDraft = () => {
    ...
    if (this.draft.uploads instanceof Array) {
      ...
      this.draft.uploads = this.draft.uploads.filter(
        upload => !uploaded.includes(upload)
      );
    }
