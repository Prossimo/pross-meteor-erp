...
const validateSalesRecord = _id => {
  ...
  if (!salesRecord)
    throw new Meteor.Error(`Not found salesRecord with _id: ${_id}`);
...
const validatePermission = (userId, salesRecord) => {
  if (
    !Roles.userIsInRole(userId, [ROLES.ADMIN, ROLES.SALES, ROLES.MANAGER]) &&
    salesRecord &&
    salesRecord.members.map(({ userId }) => userId).indexOf(userId) === -1
  ) {
...
Meteor.methods({
  removeSalesRecord({ _id, isRemoveSlack, isRemoveFolders }) {
    ...
    Meteor.defer(() => {
      ...
      isRemoveSlack &&
        slackChannel &&
        slackChannel.id &&
        Meteor.call("archiveSlackChannel", slackChannel);
    });
  },
  ...
  changeStageOfSalesRecord(salesRecordId, stage) {
    ...
    Meteor.call("sendDealStatusChangeToSlack", {
      salesRecordId,
      statusName: "stage",
      oldValue: oldStage,
      newValue: stage
  ...
  changeSubStageOfSalesRecord(salesRecordId, subStage) {
    ...
    ServerLog.info(
      JSON.stringify({
        salesRecordId,
        statusName: "sub stage",
        oldValue: oldSubStage,
        newValue: subStage
      })
    );
    Meteor.call("sendDealStatusChangeToSlack", {
  ...
  addStakeholderToSalesRecord({ _id, peopleId, addToMain }) {
    ...
    if (addToMain) {
      ...
      Conversations.update(mainConversationId, {
        $push: { participants: { peopleId } }
      });
    }
  },
  ...
  insertSalesRecord({ data, thread, isPrivateSlackChannel }) {
    ...
    const responseInviteBot = Meteor.call(
    ...
    Meteor.users
      .find({ _id: { $in: data.members }, slack: { $exists: true } })
      .forEach(user =>
    ...
    const mainConversationId = Conversations.insert({
      ...
      participants: data.stakeholders
        .filter(s => s.addToMain)
        .map(({ peopleId, isMainStakeholder }) => ({
          peopleId,
    ...
    Meteor.defer(() => {
      const folderId = prossDocDrive.createSalesRecordFolder.call({
        name: data.name,
        salesRecordId
      });
      const { webViewLink, webContentLink } = prossDocDrive.getFiles.call({
        fileId: folderId
      });
    ...
    if (thread) {
      Threads.update(
        { _id: thread._id },
        { $set: { conversationId: mainConversationId } }
      );
    ...
    HTTP.post(
      "http://78.47.83.46:8000/api/login_check",
      {
        data: {
          _username: "admin",
          _password: "12345678"
      ...
      (error, response) => {
        if (error) {
          ...
          HTTP.post(
            "http://78.47.83.46:8000/api/projects",
            {
              data: {
                project: {
  ...
  updateSalesRecord({ _id, data, thread, conversationId }) {
    ...
    if (
      sr.conversationIds &&
      sr.conversationIds.length > 0 &&
      data.stakeholders
    ) {
      Conversations.update(
        { _id: sr.conversationIds[0] },
        {
          $set: {
            participants: data.stakeholders
              .filter(s => s.addToMain)
  ...
  updateSalesRecordMembers(salesRecordId, members) {
    ...
    if (members && members.length) {
      Meteor.users
        .find({
          _id: {
            $in: members.filter(mid => salesRecord.members.indexOf(mid) == -1)
          },
          slack: { $exists: true }
        })
        .forEach(({ slack: { id } }) => {
          Meteor.call("inviteUserToSlackChannel", {
            ...salesRecord.slackChannel,
            user: id
          });
        });
    }
  ...
  updateSalesRecordStatus(salesRecordId, status) {
    ...
    const statusValue = key => {
      if (key === "teamLead") {
        ...
        if (teamLead) {
          return teamLead.slack
            ? `<@${teamLead.slack.id}|${teamLead.slack.name}>`
            : teamLead.username;
        } else {
          return null;
        }
    ...
    ServerLog.info(
      "sendDealStatusChangeToSlack data for updateSalesRecordStatus",
      {
        salesRecordId,
        statusName: Object.keys(status).join(","),
        oldValue: oldValues.join(","),
        newValue: Object.keys(status).map(key => status[key])
      }
    );
    Meteor.call("sendDealStatusChangeToSlack", {
      salesRecordId,
      statusName: Object.keys(status).join(","),
      oldValue: oldValues.join(","),
      newValue: Object.keys(status)
        .map(key => statusValue(key))
        .join(",")
    });
  },
...
export const pushConversationToSalesRecord = new ValidatedMethod({
  ...
  validate: new SimpleSchema({
    _id: SalesRecords.schema.schema("_id"),
