...
const validatePermission = (userId, project) => {
  if (
    ...
    project &&
    project.members.map(({ userId }) => userId).indexOf(userId) === -1
  ) {
...
export const createProject = new ValidatedMethod({
  ...
  validate: Projects.schema
    .pick("name", "members", "stakeholders", "nylasAccountId")
    .extend({
  ...
  run({
    ...
    if (members) {
      Meteor.users
        ...
        .forEach(({ slack: { id } }) =>
          Meteor.call("inviteUserToSlackChannel", { ...slackChannel, user: id })
        );
    }
    ...
    Meteor.defer(() => {
      // CREATE DRIVE
      const folderId = prossDocDrive.createProjectFolder.call({
        name: project.name,
        projectId
      });
  ...
  run({ _id, name, members, stakeholders, thread, conversationId }) {
    ...
    if (members && members.length) {
      Meteor.users
        .find({
          _id: {
            $in: _.pluck(members, "userId").filter(
              mid => _.pluck(project.members, "userId").indexOf(mid) == -1
            )
          },
          slack: { $exists: true }
        })
...
Meteor.methods({
  ...
  updateProjectMembers(projectId, members) {
    ...
    if (
      ...
      members.every(m => project.members.indexOf(m) > -1)
    )
      return;
