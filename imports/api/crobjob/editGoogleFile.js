...
let { startPageToken: pageToken } = Meteor.wrapAsync(
...
export const observeGoogleFile = function() {
  ...
  const getAncestors = fileId => {
    ...
    const getFileWithParent = fileId => {
      ...
      const { id, name, webViewLink, parents } = getFileSync({
        fileId,
  ...
  ChangedFiles.find()
    ...
    .forEach(({ _id }) => {
      if (!SlackFiles.findOne(_id)) {
        ...
        const project =
          SalesRecords.findOne({ folderId: { $in: folderIds } }) ||
          Projects.findOne({ folderId: { $in: folderIds } });
      ...
      if (slackFile && slackFile.channel) {
        ...
        if (nComments > slackFile.nComments) {
          text = `${nComments -
            slackFile.nComments} comments has been added to file <${
            slackFile.file.webViewLink
          }|${slackFile.file.name}>`;
          ...
          text = `file <${slackFile.file.webViewLink}|${
            slackFile.file.name
          }> has been edited`;
        }
...
export const registerCallback = function() {
  drive.changes.watch({
    ...
    resource: {
      ...
      address:
        env === "development"
          ? "https://9e1347ca.ngrok.io/notifications"
          : Meteor.absoluteUrl("notifications"),
      type: "web_hook"
    }
  });
};
