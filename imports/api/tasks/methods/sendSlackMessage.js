            ...
            if (channel) {
                ...
                switch (type) {
                    ...
                    case 'ASSIGN_TASK': {
                        ...
                        const pretext = `New task has been assigned to ${userRefer} by ${actorRefer} in ${status} board of <${title_link}|${parent.name}>`
                    ...
                    case 'UPDATE_TASK': {
                        ...
                        const actorRefer = (actor.slack && actor.slack.id) ? `<@${actor.slack.id}>` : actor.username
                        const attachments = slackClient.attachments.create({
                            pretext: `A task have been updated by ${actorRefer} in ${status} board of <${title_link}|${parent.name}>`,
                            title,
                            text,
                            color: ORANGE,
                            title_link,
                    ...
                    case 'REMOVE_TASK': {
                        const attachments = slackClient.attachments.create({
                            pretext: `A task have been removed from ${status} board of <${title_link}|${parent.name}>`,
                            title,
                            text,
                            color: RED,
                            title_link,
                    ...
                    case 'NEW_TASK': {
                        ...
                        if (user) {
                            const userRefer = (user.slack && user.slack.id) ? `<@${user.slack.id}>` : user.username
                            const actorRefer = (actor.slack && actor.slack.id) ? `<@${actor.slack.id}>` : actor.username
                            pretext = `New task has been assigned to ${userRefer} by ${actorRefer} in ${status} board of <${title_link}|${parent.name}>`
                        } else {
