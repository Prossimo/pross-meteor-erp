...
Meteor.methods({
  sendMailToSlack(message, { files, mentions } = {}) {
    ...
    if (slackMail) {
      if (
        slackMail.message_ids &&
        slackMail.message_ids.indexOf(message.id) > -1
      ) {
    ...
    const slackText = `${
      mentions && mentions.length
        ? `${mentions.map(m => `<@${m.id}|${m.name}>`).join(", ")}. `
        : ""
    }An email was sent from ${message.from[0].email} to ${to.join(", ")}`;
    ...
    if (target && conversation) {
      ...
      )}|Go to ${target.type} ${target.name}/${conversation.name}>`;
    }
